"""
YAML resolution and the tree runner for the fetch tree.
...
is called -- which is why the fetch tree can run alongside a leading, ostensive
or seek one and `ros2 node list` says which is which.
"""
...
# The ROS node name the fetch tree registers under.
NODE_NAME = "fetch_bt_node"
...
def run_tree(create_root, tree_name, default_yaml, args=None, tick_period_ms=None):
    """
    Build, set up and spin the fetch tree.
