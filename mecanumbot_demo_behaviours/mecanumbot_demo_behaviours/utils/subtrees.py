...
from mecanumbot_demo_behaviours.behaviours.dog_behaviours import (
    DogIndicateTarget,
    DogLookForFeedback,
    DogCatchAttention,
)

from mecanumbot_demo_behaviours.behaviours.LED_behaviours import (
    LEDIndicateTarget,
    LEDCatchAttention,
    LEDStop,
)
...
def create_approach_subject_subtree(
    subject_topic_name="subject_pose",
    robot_topic_name="amcl_pose",
    subj_dist_threshold=0.5,
    tgt_reach_threshold=0.2,
    approach_distance_threshold=0.3,
):
    """
    ...
    subject_to_blackboard = SubjectToBlackboard(
        name="SubjectToBlackboard", topic_name=subject_topic_name
    )
    subject_to_goal_pose = SubjectToGoalPose(name="SubjectToGoalPose")
    robot_to_blackboard = RobotToBlackboard(
        name="RobotToBlackboard", topic_name=robot_topic_name
    )
    distance_to_blackboard = DistanceToBlackboard(
        name="DistanceToBlackboard",
        threshold=subj_dist_threshold,
        target_reach_threshold=tgt_reach_threshold,
    )
    check_approach_success = CheckApproachSuccess(
        name="CheckApproachSuccess",
...
def create_approach_subject_until_success_subtree(
    subject_topic_name="subject_pose",
    robot_topic_name="amcl_pose",
    subj_dist_threshold=0.5,
    tgt_reach_threshold=0.2,
    ...
    approach_subject_subtree = create_approach_subject_subtree(
        subject_topic_name,
        robot_topic_name,
        subj_dist_threshold,
        tgt_reach_threshold,
...
def create_indicate_target_subtree(condition):
    ...
    check_subject_target_success = CheckSubjectTargetSuccess(
        name="CheckSubjectTargetSuccessIndicate"
    )
