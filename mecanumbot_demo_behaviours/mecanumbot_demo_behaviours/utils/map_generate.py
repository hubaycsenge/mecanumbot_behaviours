...
def generate_patrol_route(pgm_path, yaml_resolution, yaml_origin, num_rays=36, num_waypoints=None, min_dist_px=None, max_dist_px=None):
    """
    ...
    if num_waypoints is None:
        ...
        print(f"Auto-calculated num_waypoints: {num_waypoints} (Total free area: {free_area_m2:.1f} m^2)")
    # ------------------------
    ...
    for i, (px, py) in enumerate(waypoints_pixel):
        ...
        # Number the waypoint
        cv2.putText(display_map, str(i+1), (px + 8, py - 8), 
...
def generate_waypoints(base_name,min_dist_px=None,max_dist_px=None,num_waypoints=None):
    """
    ...
    out_path = os.path.join(description_base_path,'maps', base_name, base_name + "_waypoints.yaml")
    in_features_path = os.path.join(description_base_path, 'maps', base_name, base_name + ".yaml")
    in_map_path = os.path.join(description_base_path, 'maps', base_name, base_name + ".pgm")
